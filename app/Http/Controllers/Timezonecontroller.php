...
class Timezonecontroller extends Controller
{
	...
	{	
		...
		//Stripe key list
    public function razorpayList()
	{	
		//$settings_data=DB::table('updatekey')->first();
		$settings_data = UpdatekeyRazorpay::first();

		return view('razorpay_setting.list',compact('settings_data')); 
	}


	// Stripe Key Update
	public function razorpayStore(StoreStripeSettingEditFormRequest $request)
	{	
		/*$updateStripeKey = DB::table('updatekey')->where('stripe_id', $request->stripe_id)->update([
			'secret_key' => $request->secret_key,
			'publish_key' => $request->publish_key
		...
		$updateStripeKey = UpdatekeyRazorpay::where('razorpay_id', $request->razorpay_id)->update([
			'secret_key' => $request->secret_key,
			'publish_key' => $request->publish_key
		...
		if ($updateStripeKey) {
			return redirect('/setting/razorpay/list')->with('message','Successfully Updated');		
		} else {
			return redirect('/setting/razorpay/list')->with('error','Not Updated');
		}
	}
}	
