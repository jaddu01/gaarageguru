...
class InvoiceController extends Controller
{
	...
	public function get_sales_customer(Request $request)
	{
		$type = $request->type;
		if($type == 1){
			$customer = DB::table('tbl_sales')->select('customer_id')->groupBy('customer_id')->get()->toArray();
    ...
    //get jobcard number for which service has not generated Invoice
    public function get_jobcard_no_new(Request $request)
    {
        $cus_id = $request->cus_name;

        $currentUser = User::where([['soft_delete',0],['id','=',Auth::User()->id]])->orderBy('id','DESC')->first();
        $adminCurrentBranch = BranchSetting::where('id','=',1)->first();
        $getJobcardNoFromServiceTbl = Service::where('customer_id', $cus_id)->where('job_no', 'LIKE', 'J%')->groupBy('job_no');

        if (isAdmin(Auth::User()->role_id)){
            $getJobcardNoFromServiceTbl = $getJobcardNoFromServiceTbl->where('done_status', 1)->where('branch_id', $adminCurrentBranch->branch_id)->get();
            //$getJobcardNoFromServiceTbl = DB::table('tbl_services')->where([['customer_id','=', $cus_id],['done_status','=',1],['job_no','like','J%'], ['branch_id',$adminCurrentBranch->branch_id]])->get()->toArray();
//            $invoiceTblJobcardNo = DB::table('tbl_invoices')->where([['customer_id','=', $cus_id],['job_card','like','J%'],['type', '=', 0], ['branch_id',$adminCurrentBranch->branch_id]])->get()->toArray();
            $invoiceTblJobcardNo = Invoice::where('customer_id',$cus_id)->where('job_card','like','J%')->where('type',0)->where('branch_id',$adminCurrentBranch->branch_id)->get()->toArray();
        }
        elseif (getUsersRole(Auth::user()->role_id) == 'Customer') {
//            $getJobcardNoFromServiceTbl = DB::table('tbl_services')->where([['customer_id','=', $cus_id],['done_status','=',1],['job_no','like','J%']])->get()->toArray();
            $getJobcardNoFromServiceTbl = $getJobcardNoFromServiceTbl->where('done_status', 1)->get();
            $invoiceTblJobcardNo = Invoice::where('customer_id', $cus_id)->where('type', 0)->where('job_no', 'LIKE', 'J%')->get()->toArray();
            //$invoiceTblJobcardNo = DB::table('tbl_invoices')->where([['customer_id','=', $cus_id],['job_card','like','J%'],['type', '=', 0]])->get()->toArray();
        }
        else{
            $getJobcardNoFromServiceTbl = $getJobcardNoFromServiceTbl->where('done_status', 1)->where('branch_id', $adminCurrentBranch->branch_id)->get();
            $invoiceTblJobcardNo = Invoice::where('customer_id', $cus_id)->where('type', 0)->where('job_no', 'LIKE', 'J%')->where('branch_id', $adminCurrentBranch->branch_id)->get()->toArray();
//            $getJobcardNoFromServiceTbl = DB::table('tbl_services')->where([['customer_id','=', $cus_id],['done_status','=',1],['job_no','like','J%'], ['branch_id',$currentUser->branch_id]])->get()->toArray();
//            $invoiceTblJobcardNo = DB::table('tbl_invoices')->where([['customer_id','=', $cus_id],['job_card','like','J%'],['type', '=', 0], ['branch_id',$currentUser->branch_id]])->get()->toArray();
        }
...
//        }
        ...
        $html = "";
        foreach ($getJobcardNoFromServiceTbl as $getJobcardServiceTbls) {
            $html .= '<option class="invoice_job_number" value="'.$getJobcardServiceTbls->job_no.'" >'.$getJobcardServiceTbls->job_no.'</option>';
        }
