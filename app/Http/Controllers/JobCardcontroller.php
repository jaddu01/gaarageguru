...
class JobCardcontroller extends Controller
{
	...
	public function store(Request $request)
	{
		...
		{
		    $jobcard_details = JobcardDetail::where('service_id', $service_id)->first();
		    if ($jobcard_details){
                $jobcard_details->update(['out_date' => $odate, 'kms_run' => $kms]);
            }
		}

		Service::find($service_id)->update(['done_status'=>1]);
        $jobcard_details = JobcardDetail::where('service_id', $service_id)->first();
        if ($jobcard_details){
            $jobcard_details->update(['done_status' => 1]);
        }
